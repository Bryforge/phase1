...
Current status: B1 read-only detection has an initial script and tests. B2 dry-run assembly has an initial script, tests, limitations, validation report, output review, and focused test-suite command bundle, but remains dry-run preview only until the focused B2 test suite passes locally or in CI.
...
10. Add B2 limitations, validation, output review, and test-suite docs.
11. Add the [`Base1 x86_64 boot support roadmap`](X86_64_BOOT_SUPPORT_ROADMAP.md).
12. Add the [`Base1 image-builder design`](BASE1_IMAGE_BUILDER.md).
13. Add the [`Base1 installer and recovery design`](INSTALLER_RECOVERY.md).
14. Add the [`Base1 installer dry-run design`](BASE1_INSTALLER_DRY_RUN.md).
15. Add system-surface command stubs behind safe defaults.
16. Add hardware-target checklists.
17. Add read-only boot readiness status reporting.
