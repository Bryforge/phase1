...
>
> **Validation:** Links to the trust model, crypto policy roadmap, crypto implementation plan, crypto registry, crypto profiles index, crypto operator command plan, crypto config schema, crypto algorithm template, claims policy, review guide, and manual roadmap added in this repository.
>
...
5. [`crypto-profiles/README.md`](crypto-profiles/README.md) — planning index for operator-selectable cryptographic profiles.
6. [`CRYPTO_OPERATOR_COMMANDS.md`](CRYPTO_OPERATOR_COMMANDS.md) — planning design for future operator-facing crypto policy commands.
7. [`CRYPTO_CONFIG_SCHEMA.md`](CRYPTO_CONFIG_SCHEMA.md) — planning schema for future scoped crypto policy configuration.
8. [`CRYPTO_ALGORITHM_TEMPLATE.md`](CRYPTO_ALGORITHM_TEMPLATE.md) — required template for documenting cryptographic algorithms, designs, providers, and profile components.
9. [`DOCS_CLAIMS.md`](DOCS_CLAIMS.md) — allowed wording, disallowed wording, status labels, and required evidence.
10. [`REVIEW_GUIDE.md`](REVIEW_GUIDE.md) — practical reviewer checklist for safety-sensitive documentation.
11. [`../MANUAL_ROADMAP.md`](../MANUAL_ROADMAP.md) — full Codex architecture, safety model, glossary, and launch plan.

## Review order for safety-sensitive docs

Use this order when reviewing pages about host tools, Base1, recovery, rollback, hardware, installers, image writing, cryptographic policy, cryptographic algorithms, cryptographic profiles, crypto operator commands, crypto configuration, crypto implementation, or security claims:
...
13. Confirm crypto implementation follows [`CRYPTO_IMPLEMENTATION_PLAN.md`](CRYPTO_IMPLEMENTATION_PLAN.md) before it protects real data or runtime control points.
14. Confirm no page claims secure OS replacement, bootable Base1 release, installer readiness, daily-driver readiness, recovery completion, cryptographic completeness, audit completion, certification, or quantum safety without linked evidence.
15. Use [`REVIEW_GUIDE.md`](REVIEW_GUIDE.md) before approving safety-sensitive documentation.
