...
#
# Main design defaults: color/native UI, UTF-8/Japanese-ready, stable channel,
# safety enabled, and auto-boot directly into Phase1. ASCII is not the default;
# it is available only as an explicit fallback boot entry.
...
partition_one() { case "$1" in /dev/nvme[0-9]n[0-9]|/dev/mmcblk[0-9]) printf '%sp1\n' "$1" ;; /dev/sd[a-z]|/dev/vd[a-z]|/dev/hd[a-z]) printf '%s1\n' "$1" ;; *) fail "use a whole-disk path such as /dev/sdb, not a partition" ;; esac; }
find_busybox() { if [ -n "$BUSYBOX" ]; then printf '%s\n' "$BUSYBOX"; return; fi; for p in /bin/busybox /usr/bin/busybox /bin/busybox.static /usr/bin/busybox.static; do [ -x "$p" ] && { printf '%s\n' "$p"; return; }; done; printf '\n'; }
find_phase1_bin() { if [ -n "$PHASE1_BIN" ]; then printf '%s\n' "$PHASE1_BIN"; return; fi; for p in target/release/phase1 target/debug/phase1; do [ -x "$p" ] && { printf '%s\n' "$p"; return; }; done; printf '\n'; }
copy_one_file() { src="$1"; destroot="$2"; [ -e "$src" ] || return 0; dst="$destroot$src"; mkdir -p "$(dirname "$dst")"; cp -L "$src" "$dst"; }
copy_libs_for_binary() { bin="$1"; dest="$2"; command -v ldd >/dev/null 2>&1 || return 0; ldd "$bin" 2>/dev/null | while IFS= read -r line; do lib=""; case "$line" in *'=> /'*) lib="$(printf '%s\n' "$line" | awk '{print $3}')" ;; /*) lib="$(printf '%s\n' "$line" | awk '{print $1}')" ;; esac; [ -n "$lib" ] && [ -e "$lib" ] && copy_one_file "$lib" "$dest"; done; }
copy_common_loaders() { dest="$1"; for loader in /lib64/ld-linux-x86-64.so.2 /lib/x86_64-linux-gnu/ld-linux-x86-64.so.2 /lib/ld-linux.so.2 /lib/i386-linux-gnu/ld-linux.so.2; do [ -e "$loader" ] && copy_one_file "$loader" "$dest"; done; }
...
for cmd in sudo parted mkfs.vfat mount umount sync mkdir cp tee grub-install sha256sum find cpio gzip chmod ln date stat file grep awk; do need_cmd "$cmd"; done
...
for app in sh ash mount umount mkdir cat echo ls dmesg uname sleep reboot poweroff halt mknod stty clear printf grep awk wc find sort sha256sum head tail date ps free df sync env file; do ln -sf busybox "$ROOTFS/bin/$app" 2>/dev/null || true; done
...
for path in README.md Cargo.toml phase1 start_phase1 FEATURE_STATUS.md PHASE1_NATIVE_LANGUAGE.md; do [ -f "$path" ] && cp "$path" "$ROOTFS/phase1/repo/$path"; done
...
BASE1_B42_ASCII_DEFAULT=0
BASE1_B42_ASCII_FALLBACK=1
BASE1_B42_AUTO_BOOT_ACTIVE=$PHASE1_AUTO_BOOT
BASE1_B42_STABLE_DEFAULT=1
BASE1_B42_SAFE_DEFAULT=1
BASE1_B42_HOST_TOOLS_DEFAULT=0
BASE1_B42_EDGE_DEFAULT=0
...
if [ "$ASCII_FALLBACK" = "1" ]; then echo "phase1 6.0.0 ready"; echo "Base1 B42 ASCII fallback console"; else echo "phase1 6.0.0 ready"; echo "Base1 B42 native color + Japanese UTF-8 console"; fi
echo "stable: enabled"
echo "safety: enabled"
echo "ascii default: no; ascii fallback: yes"
echo "auto boot main design: $PHASE1_AUTO_BOOT"
...
while true; do
  ...
  case "$cmd" in
    ...
    status|s) uname -a; echo "term=$TERM color=${PHASE1_COLOR_MODE:-} ascii=${PHASE1_ASCII:-} auto=${PHASE1_AUTO_BOOT:-} lang=$LANG safety=$PHASE1_SAFE_MODE stable=$PHASE1_STABLE" ;;
    shell|sh) /bin/sh ;;
...
printf 'PHASE1 BASE1 B42 STABLE SAFE COLOR UTF-8 AUTOBOOT USB WRITER\n\n'
...
menuentry "Start Phase1 Stable Safe Color UTF-8" {
    clear
    echo "phase1 6.0.0 ready"
    echo "B42 stable safe color UTF-8 default autoboot"
    linux /boot/phase1/vmlinuz console=tty0 rdinit=/init init=/init nomodeset quiet loglevel=0 panic=0 phase1.stable=1 phase1.safe=1 phase1.ascii=0 phase1.autoboot=1 phase1.utf8=1
...
menuentry "Start Phase1 ASCII Safe Fallback" {
    ...
    linux /boot/phase1/vmlinuz console=tty0 rdinit=/init init=/init nomodeset quiet loglevel=0 panic=0 phase1.stable=1 phase1.safe=1 phase1.ascii=1 phase1.autoboot=0 phase1.utf8=1
    initrd /boot/phase1/phase1-b42-native-stable-safe-color-utf8.img
    boot
}
...
BASE1_B42_AUTO_BOOT_DEFAULT=1
EOF
printf '\nDONE: B42 stable safe color UTF-8 autoboot USB prepared on %s\n' "$USB"
