#!/usr/bin/env sh
# Record the best observed X200 B7 external USB boot result.
#
# Usage:
...
case "$RESULT" in
  boot_started|phase1_marker_seen|blocked_after_initrd_load|blocked_after_kernel_load|blocked|failed) : ;;
  *)
    cat <<'USAGE'
Usage:
...
printf 'Recorded X200 B7 evidence result: %s\n' "$RESULT"
printf 'Evidence file: %s\n\n' "$REPORT"
cat "$REPORT"

printf '\nNext local commit command, if desired:\n'
printf '  git add -f %s && git commit -m "Record X200 B7 %s"\n' "$REPORT" "$RESULT"
