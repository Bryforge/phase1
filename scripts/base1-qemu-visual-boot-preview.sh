...
generate_qemu_splash() {
  have sips || fail "missing sips; this macOS QEMU preview uses sips to size the boot splash"
  [ -f "$SPLASH_SOURCE" ] || fail "missing splash asset: $SPLASH_SOURCE"

  # Use the real public Phase1 word-mark asset, but fit it before GRUB sees it.
  # The result is a smallish centered 1024x768 splash, not an oversized crop.
  sips -Z "$QEMU_SPLASH_MAX_EDGE" "$SPLASH_SOURCE" --out "$QEMU_SPLASH_FIT" >/dev/null
  sips --padToHeightWidth "$QEMU_SPLASH_HEIGHT" "$QEMU_SPLASH_WIDTH" --padColor 000000 \
    "$QEMU_SPLASH_FIT" --out "$QEMU_SPLASH" >/dev/null
}
...
EOF
  ...
  printf 'splash: assets/phase1_word.png fitted to %sx%s with max edge %s\n' "$QEMU_SPLASH_WIDTH" "$QEMU_SPLASH_HEIGHT" "$QEMU_SPLASH_MAX_EDGE"
