...
fn reorganization_verifier_is_read_only() {
    ...
    assert!(
        script.contains("Base1 reorganization verification bundle"),
        "{script}"
    );
...
fn reorganization_verifier_runs_required_gates() {
    ...
    for command in [
        ...
        assert!(
            ...
            "missing verifier command {command}: {script}"
        );
    }
}
...
fn reorganization_verifier_handles_missing_cargo_explicitly() {
    ...
    assert!(
        script.contains(
            "run cargo test --all-targets on a Rust-capable host before claiming full readiness"
        ),
        "{script}"
    );
}
