const IMAGE_RELEASE_NOTES: &str = "docs/base1/releases/RELEASE_BASE1_RECOVERY_USB_IMAGE_READONLY_V1.md";

#[test]
fn recovery_usb_image_release_notes_record_checkpoint_status() {
    let doc = std::fs::read_to_string(IMAGE_RELEASE_NOTES)
...
fn recovery_usb_image_surfaces_link_release_notes() {
    ...
    assert!(summary.contains("RELEASE_BASE1_RECOVERY_USB_IMAGE_READONLY_V1.md"), "{summary}");
    assert!(index.contains("RELEASE_BASE1_RECOVERY_USB_IMAGE_READONLY_V1.md"), "{index}");
    assert!(command_index.contains(IMAGE_RELEASE_NOTES), "{command_index}");
    assert!(readme.contains("RELEASE_BASE1_RECOVERY_USB_IMAGE_READONLY_V1.md"), "{readme}");
    assert!(releases.contains("RELEASE_BASE1_RECOVERY_USB_IMAGE_READONLY_V1.md"), "{releases}");
}
