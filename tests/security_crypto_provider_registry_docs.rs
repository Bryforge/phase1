...
fn crypto_provider_registry_defines_purpose_and_security_goal() {
    ...
    assert!(registry.contains("Phase1 crypto provider registry"), "{registry}");
    assert!(
        registry.contains("as secure as possible while maintaining practical usability"),
        "{registry}"
    );
    assert!(
        registry.contains("Listing a provider here does not approve it for production security claims."),
        "{registry}"
    );
}
...
fn crypto_provider_registry_lists_required_provider_metadata() {
    ...
    for field in [
        ...
        assert!(registry.contains(field), "missing provider metadata field {field}: {registry}");
    }
}
...
fn crypto_provider_registry_defines_status_labels() {
    ...
    for status in [
        ...
        assert!(registry.contains(status), "missing provider status {status}: {registry}");
    }
}
...
fn crypto_provider_registry_requires_fail_closed_selection() {
    ...
    for rule in [
        ...
        assert!(registry.contains(rule), "missing provider selection rule {rule}: {registry}");
    }
}
...
fn crypto_provider_registry_requires_review_before_profile_use() {
    ...
    for item in [
        ...
        assert!(registry.contains(item), "missing provider review item {item}: {registry}");
    }
}
...
fn crypto_provider_registry_preserves_non_claims() {
    ...
    assert!(registry.contains("does not make Phase1 or Base1 cryptographically complete"), "{registry}");
