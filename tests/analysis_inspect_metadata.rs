...
fn run_phase1(input: &str) -> String {
    ...
    let run_dir = std::env::temp_dir().join(format!(
        "phase1-analysis-inspect-metadata-{}-{nonce}-{seq}",
        process::id()
    ));
    let _ = fs::remove_dir_all(&run_dir);
    fs::create_dir_all(&run_dir).expect("create analysis inspect metadata test dir");
...
#[test]
fn analyze_inspect_reports_loaded_vfs_metadata_without_execution() {
    let output = run_phase1(
        "echo 'phase1-sample' > sample.bin\nanalyze load sample.bin\nanalyze inspect sha256-016644b74537\nexit\n",
    );
...
#[test]
fn analyze_inspect_reports_missing_and_unknown_ids_without_execution() {
    let output = run_phase1("analyze inspect\nanalyze inspect sha256-deadbeef0000\nexit\n");
