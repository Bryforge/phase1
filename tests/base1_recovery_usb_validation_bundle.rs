...
#[test]
fn recovery_usb_validation_bundle_runs_read_only_previews() {
    let output = Command::new("sh")
        .arg("scripts/base1-recovery-usb-validate.sh")
        .output()
        .expect("run recovery USB validation bundle");
    ...
    assert!(
        text.contains("base1 recovery USB validation bundle complete"),
        "{text}"
    );
}

#[test]
fn recovery_usb_validation_bundle_help_is_read_only() {
    let output = Command::new("sh")
        .arg("scripts/base1-recovery-usb-validate.sh")
        .arg("--help")
        .output()
        .expect("run recovery USB validation help");
...
#[test]
fn recovery_usb_validation_bundle_avoids_mutating_tools_and_sensitive_terms() {
    let script = std::fs::read_to_string("scripts/base1-recovery-usb-validate.sh")
        .expect("recovery USB validation bundle");
