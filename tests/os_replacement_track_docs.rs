...
fn readme_links_phase1_operating_system_track() {
    ...
    assert!(readme.contains("not a current drop-in OS replacement"), "{readme}");
}
...
fn os_track_roadmap_documents_guarded_stages() {
    ...
    assert!(doc.contains("Base1 provides the minimal trusted host foundation"), "{doc}");
    ...
    assert!(doc.contains("Stage 4: Phase1-owned system surface"), "{doc}");
    assert!(doc.contains("Stage 5: Hardware targets"), "{doc}");
    assert!(doc.contains("No claim that the current terminal console replaces a kernel"), "{doc}");
}
