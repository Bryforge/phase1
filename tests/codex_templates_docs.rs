...
fn codex_template_docs_exist() {
    for path in [
        ...
        assert!(
            ...
            "missing Codex template doc: {path}"
        );
    }
}
...
fn docs_index_links_codex_templates() {
    let index = fs::read_to_string("docs/README.md").expect("docs index should be readable");
...
fn template_index_links_core_templates() {
    let index =
        fs::read_to_string("docs/templates/README.md").expect("templates index should be readable");
    ...
    for expected in [
        ...
        assert!(
            ...
            "templates index missing {expected}"
        );
    }
}
...
fn status_blocks_include_required_statuses() {
    ...
    for expected in [
        ...
        assert!(
            status_blocks.contains(expected),
            "missing status template: {expected}"
        );
    }
}
