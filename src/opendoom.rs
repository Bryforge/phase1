...
impl Game {
    ...
    fn step(&mut self, raw: &str) -> String {
        ...
        let acted = match command.as_str() {
            ...
            "quit" | "exit" => {
                self.done = true;
                out.push_str("openDoom: session ended.\n");
                false
            }
        ...
        if acted && !self.done {
            ...
            if self.hp <= 0 {
                self.done = true;
                self.won = false;
                out.push_str("You are down. openDoom run failed.\n");
            }
        }
    ...
    fn try_move(&mut self, dir: Dir, out: &mut String) -> bool {
        ...
        if self.enemies.iter().any(|enemy| enemy.x == nx && enemy.y == ny) {
            out.push_str("A demon blocks that tile. Fire first.\n");
            return true;
        }
        self.player_x = nx;
        self.player_y = ny;
    ...
    fn fire(&mut self, out: &mut String) -> bool {
        ...
        if self.enemies[target_idx].hp <= 0 {
            ...
            self.kills += 1;
            out.push_str(&format!("Direct hit. Demon cleared at {},{}.\n", enemy.x, enemy.y));
        } else {
            out.push_str("Direct hit. The demon staggers.\n");
        }
        true
    }
    ...
    fn enemy_turn(&mut self, out: &mut String) {
        ...
        for idx in 0..self.enemies.len() {
            ...
            if distance == 1 {
                self.hp -= 5;
                out.push_str("A demon claws you for 5 damage.\n");
                continue;
            }
            if distance > 5 {
                continue;
    ...
    fn check_goal(&mut self, out: &mut String) {
        if (self.player_x, self.player_y) == (self.exit_x, self.exit_y) {
            if self.enemies.is_empty() {
                self.done = true;
                self.won = true;
                out.push_str("Exit reached. openDoom arena cleared.\n");
            } else {
                out.push_str("The exit is sealed until every demon is cleared.\n");
            }
        }
    }
    ...
    fn render(&self) -> String {
        let mut out = String::new();
        out.push_str("\nphase1 openDoom // original text-mode arena\n");
        out.push_str("asset mode: clean-room ASCII, no external WAD required\n");
        out.push_str(&format!(
...
pub fn run(args: &[String]) -> String {
    match args.first().map(String::as_str) {
        ...
        Some(other) => format!("opendoom: unknown option '{other}'\n{}", help_text()),
    }
}
...
fn run_script(commands: &[&str]) -> String {
    let mut game = Game::new();
    let mut out = String::from("phase1 openDoom scripted run\n");
...
fn help_text() -> String {
    "phase1 openDoom\nusage : opendoom [start|demo|script <commands...>|help]\nplay  : run `opendoom start`, then use w/a/s/d, fire, map, help, quit\ngoal  : clear the demons, grab supplies, and reach E\nnote  : original ASCII arena; no proprietary game assets or WAD files are used\n".to_string()
}
...
mod tests {
    ...
    #[test]
    fn demo_renders_open_doom_arena() {
        let out = run(&["demo".to_string()]);
        assert!(out.contains("phase1 openDoom"));
        assert!(out.contains("@"));
        assert!(out.contains("M"));
        assert!(out.contains("asset mode: clean-room ASCII"));
    }
    ...
    #[test]
    fn pickups_change_player_state() {
        let out = run(&[
            "script".to_string(),
            "s".to_string(),
            "s".to_string(),
        ...
        assert!(out.contains("Ammo cache") || out.contains("phase1 openDoom"));
    }
}
