// =====================================================
// kernel.rs — Simulated kernel primitives for phase1 v1.2.0
// =====================================================
// Educational core: VFS with permissions, preemptive scheduler with CR3/CR4/PCID, PCIe enumeration.
// Fully self-contained module for clean separation from shell/userland.
...
pub struct Vfs {
    root: VfsNode,
    pub cwd: PathBuf,   // exposed for shell compatibility
}
...
impl Vfs {
    pub fn new() -> Self {
        ...
        proc_children.insert("version".to_string(), VfsNode::File { content: "phase1 v1.2.0 (Advanced Educational Build 2026-05-03)".to_string(), perm: 4 });
...
impl Scheduler {
    pub fn new() -> Self {
        ...
        let _ = sched.spawn("phase1-shell", std::process::id(), "phase1 v1.2.0", 8192, false, 0);
        sched
    }
    ...
    pub fn top(&self) -> String {
        format!("top — phase1 kernel simulator ({} processes)\nUse 'ps' for full list. Educational demo only.", self.processes.iter().flatten().count())
    }
    ...
    pub fn jobs(&self) -> String {
        "No background jobs (simulated)".to_string()
    }
