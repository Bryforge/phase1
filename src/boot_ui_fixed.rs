...
impl BootConfig {
    pub fn detected_defaults() -> Self {
        ...
        if mobile {
            ...
            set_device_mode(default_large_screen_device());
        }
        config
    }
    ...
    fn load_saved() -> Option<Self> {
        ...
        if !saw_device_mode && !config.mobile_mode {
            set_device_mode(default_large_screen_device());
        } else if config.mobile_mode {
            set_device_mode(DeviceMode::Mobile);
        }
        config.normalize_channel();
        Some(config)
...
pub fn configure_boot(version: &str) -> BootSelection {
    ...
    if let Some(_guard) = CbreakModeGuard::enter() {
        configure_boot_live(version, &mut config)
    } else {
...
pub fn print_quick_boot(version: &str, config: BootConfig) {
    ...
    outln(&value(config, &format!("[quick] profile={} :: shell armed :: clock {}", config.profile_name(), short_clock_utc())));
    outln("");
}
...
fn configure_boot_live(version: &str, config: &mut BootConfig) -> BootSelection {
...
fn print_boot_card(version: &str, config: BootConfig, selector: bool, notice: Option<&str>) {
    ...
    outln(&card_line(config, width, &format!("node TOKYO-01 | clock {}", clock_utc())));
    ...
    for row in splash_info(version, config) { outln(&card_line(config, width, &row)); }
    if selector {
        ...
        outln(&card_line(config, width, "help dash sysinfo security"));
...
fn prompt_status_bar() -> String {
    ...
    let padded = pad_visible(&clip_visible(&raw, width), width);
...
fn ready_line(desktop: bool) {
    if color_enabled() {
        let colors = palette(active_theme());
        if desktop {
            outln(&format!("{}[ready]{} systems synced {GRAY}:: operator shell armed :: HUD clock active{RESET}", colors.ready, RESET));
        } else {
            outln(&format!("{}[ready]{} systems synced", colors.ready, RESET));
        }
    } else if desktop {
        outln("[ready] systems synced :: operator shell armed :: HUD clock active");
    } else {
...
fn default_large_screen_device() -> DeviceMode {
    if terminal_width() >= 120 { DeviceMode::Desktop } else { DeviceMode::Laptop }
}
...
struct CbreakModeGuard { original: Option<String> }

impl CbreakModeGuard {
    fn enter() -> Option<Self> {
        if std::env::var_os("PHASE1_COOKED_INPUT").is_some() { return None; }
        let original = stty(&["-g"]).ok().filter(|raw| !raw.trim().is_empty())?;
        if stty(&["cbreak", "-echo", "min", "0", "time", "10"]).is_err() { return None; }
        let _ = stty(&["opost", "onlcr"]);
        Some(Self { original: Some(original.trim().to_string()) })
    }
}

impl Drop for CbreakModeGuard { fn drop(&mut self) { if let Some(original) = self.original.take() { let _ = stty(&[&original]); } } }
...
#[cfg(test)]
mod tests {
    use super::{clock_utc, display_version, parse_bool, prompt_status_bar, strip_ansi, BootConfig, ThemePalette};
    ...
    #[test]
    fn prompt_status_bar_contains_clock() {
