...
fn configure_boot_cooked(version: &str, config: &mut BootConfig, boot_stamp: &str) -> BootSelection {
    ...
    let mut notice = Some("boot time captured; clock is static to avoid redraw glitches".to_string());
    loop {
...
fn handle_boot_input(config: &mut BootConfig, input: &str, notice: &mut Option<String>) -> Option<BootSelection> {
    match input.trim().to_ascii_lowercase().as_str() {
        ...
        "2" | "c" | "color" | "colour" | "neon" => { config.color = !config.color; *notice = Some("color output toggled".to_string()); None }
        "3" | "a" | "ascii" => { config.ascii_mode = !config.ascii_mode; *notice = Some("ASCII compatibility toggled".to_string()); None }
        "4" | "s" | "safe" | "safe-mode" | "shield" => { config.safe_mode = !config.safe_mode; *notice = Some("safe shield toggled".to_string()); None }
        ...
        "t" | "trust" | "trusted" | "host-tools" | "hosttools" => { config.host_tools = !config.host_tools; *notice = Some("trust host gate toggled; SHIELD must be off for host tools".to_string()); None }
        "p" | "persist" | "persistent" | "persistent-state" | "vault" => { config.persistent_state = !config.persistent_state; *notice = Some("persistent state toggled".to_string()); None }
...
fn print_preboot(version: &str, config: BootConfig, notice: Option<&str>, boot_stamp: &str) {
    print!("\x1b[2J\x1b[H");
    if config.color && !config.ascii_mode { print!("{BOLD}"); }
    print_boot_card(version, config, true, notice, boot_stamp);
    outln(&value(config, "Enter=boot  6=device  l=laptop  w=desktop  t=trust  h=help"));
}
...
fn print_boot_card(version: &str, config: BootConfig, selector: bool, notice: Option<&str>, boot_stamp: &str) {
    let width = card_width(config);
    outln("");
    outln(&card_top(config, width));
    outln(&card_line(config, width, &console_title(config)));
    outln(&card_line(config, width, &format!("node TOKYO-01 | boot {boot_stamp}")));
    outln(&card_rule(config, width));
    for row in skyline_rows(config) { outln(&card_line(config, width, &row)); }
    ...
    if selector {
        ...
        outln(&card_line(config, width, "help dash sysinfo security opslog"));
        outln(&card_line(config, width, "theme linux preview matrix audit ps"));
...
fn skyline_rows(config: BootConfig) -> Vec<String> {
    ...
    vec![
        ...
        format!("ui device {} :: kern/vfs/proc/net/audit/lang", device_mode(config).name()),
    ]
}
...
fn prompt_status_bar() -> String {
    ...
    let device = std::env::var("PHASE1_DEVICE_MODE").unwrap_or_else(|_| device_mode(config).name().to_string());
...
fn ready_line(desktop: bool) {
    if color_enabled() {
        let colors = palette(active_theme());
        if desktop {
            outln(&format!("{}[ready]{} systems synced {GRAY}:: operator shell armed :: ops log active{RESET}", colors.ready, RESET));
        } else {
...
fn card_top(config: BootConfig, width: usize) -> String {
    if config.color && !config.ascii_mode { format!("{}╭{}╮{RESET}", palette(active_theme_for_config(config)).border, "─".repeat(width)) } else { format!("+{}+", "-".repeat(width)) }
}
...
fn device_mode(config: BootConfig) -> DeviceMode {
    ...
    std::env::var("PHASE1_DEVICE_MODE").ok().and_then(|raw| DeviceMode::parse(&raw)).filter(|mode| *mode != DeviceMode::Mobile).unwrap_or_else(default_large_screen_device)
}

fn default_large_screen_device() -> DeviceMode { if terminal_width() >= 110 { DeviceMode::Desktop } else { DeviceMode::Laptop } }
...
fn color_enabled() -> bool { default_color_enabled() }
...
fn locale_is_utf8() -> bool {
    ["LC_ALL", "LC_CTYPE", "LANG"].iter().filter_map(|name| std::env::var(name).ok()).any(|value| {
...
fn terminal_width() -> usize {
    if let Ok(raw) = std::env::var("COLUMNS") {
        if let Ok(width) = raw.parse::<usize>() { return width.max(32); }
    }
    stty(&["size"])
        .ok()
        .and_then(|raw| raw.split_whitespace().nth(1).and_then(|value| value.parse::<usize>().ok()))
        .unwrap_or(80)
        .max(32)
}

fn detect_mobile_terminal() -> bool {
    ["TERM_PROGRAM", "TERM", "SSH_CLIENT", "PHASE1_DEVICE"].iter().filter_map(|name| std::env::var(name).ok()).any(|value| {
        let upper = value.to_ascii_uppercase();
        ["IPHONE", "ANDROID", "BLINK", "ISH", "TERMUX", "MOBILE"].iter().any(|needle| upper.contains(needle))
    })
}

fn static_boot_stamp() -> String { clock_utc() }
fn clock_utc() -> String {
...
fn clip_visible(text: &str, width: usize) -> String {
    ...
    if plain.chars().count() <= width { text.to_string() } else { plain.chars().take(width).collect() }
}
...
#[cfg(test)]
mod tests {
    use super::{clock_utc, default_large_screen_device, display_version, parse_bool, prompt_status_bar, strip_ansi, BootConfig, DeviceMode, ThemePalette};
