...
This prevents accidental credential prompts during clone, status, or pull operations.
...
- Repository names are validated to block path traversal and shell metacharacter-style names.
...
- Host command output is bounded and redacts common token/password/authorization markers.
- Build artifacts are ignored by Git through `phase1.workspace/` in `.gitignore`.
